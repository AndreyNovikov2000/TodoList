...
class CreateListViewController: UIViewController {
    ...
    func saveTask() {
        ...
        detailList?.isNotificate = false
        
        if components.hasDate() {
            
            let date = calendar.date(from: components)
            detailList?.notificationDate = date
            detailList?.isNotificate = true
            model.setNotificationWith(with: detailList, components: components)
        }
